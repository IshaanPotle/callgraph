...
independently. Cost is fixed at 24 calls and does not grow with the corpus, which is 2.8% of the
bill.
...
measured on a calibration sample and smoothed toward a pessimistic prior, so a column that rejected
...
catches 31.1%, slightly more and at no cost, while the funnel's critic accounts for $3.82 of the
...
About 9,100 lines, a fifth of it comments, because the reasoning behind a threshold is worth more
than the threshold. `src/config.ts` is the clearest example: every knob in the system in one file,
each one with the argument for its value rather than just the value.
...
Built for the voiceops Founding AI Engineer posting. TypeScript end to end, no agent framework. The
orchestration is `src/pipeline/run.ts`, 142 lines of ordinary code, because a four-stage pipeline
with a concurrency limit does not need a framework and is much easier to instrument without one.
